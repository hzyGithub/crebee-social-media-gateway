...
| `timing` | number | 是 | 秒级时间戳；`0` 为立即发布，定时为 2 小时后至 15 天内 |
| `taskId` | string | 否（运行时必传） | 任务 ID |
| `videoInfo` | `{ duration: number; width: number; height: number; size: number; thumbnail?: string }` | 否 | 视频信息 |
| `tid` | `{ fenqu_id: number; fenqu_name: string }` | 是 | 分区，通过 `bilibili/getVideoCategory` 获取 |
| `copyright` / `source` | number / string | 是 | 版权声明 / 来源；运行时转载时 `copyright` 为 `2` 且 `source` 应含链接 |
| `tags` | string[] | 是 | 标签 |
| `cover43` | string | 否 | 4:3 本地封面路径 |
| `pubType` | number | 否 | 发布类型；运行时 `1` 为立即发布、`0` 为草稿 |
| `topic` | `{ show_activity_icon: boolean; topic_name: string; arc_play_vv: number; topic_id: number; topic_description: string; mission_id: number; activity_text: string; activity_description: string; similarity_score: number; final_score: number }` | 否 | 活动话题，通过 `bilibili/getActivityTopicList` 获取；`type_pid` 传所选分区的 `fenqu_id` |
| `mentions` | `{ friend_avatar: string; friend_follower_count: number; friend_name: string; friend_uid: string }[]` | 否 | 提及用户，通过 `bilibili/getFriend` 获取 |
| `erchuang` / `self_captions` / `dolby_voice` / `danmu` / `comments` / `curated_comments` | number | 否 | 发布设置；运行时使用 `0` 或 `1` |
| `fans_desc` | string | 否 | 粉丝动态描述 |

**最小参数校验示例：**

```json
{ "title": "示例视频", "desc": "", "videoPath": "/tmp/video.mp4", "coverPath": "/tmp/cover.jpg", "timing": 0, "taskId": "task-1", "tid": { "fenqu_id": 160, "fenqu_name": "生活" }, "copyright": 1, "source": "https://example.com/source", "tags": ["标签"] }
```
...
| `taskId` | string | 否（运行时必传） | 任务 ID |
| `pubType` / `original` | number | 否 | 发布类型 / 原创标识；运行时分别使用 `1` 或 `0` |
