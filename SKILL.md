...
description: |
  CreBee 社交媒体网关。当用户需要管理社交媒体账号、发布内容（视频/图文/文章）、获取数据分析、访问粉丝画像、搜索话题/活动/音乐、或与抖音、B站、小红书、快手等平台交互时触发此技能。适用于 AI Agent 自动化管理中国社交媒体平台运营。
...
统一 HTTP API 网关，让 AI Agent 自动化管理中国主流社交媒体平台。

## 使用前提

本 Skill 通过用户电脑上的 CreBee 桌面客户端提供能力，不包含独立的云端服务。使用前必须：

1. 访问 [CreBee 下载页](https://www.crebee.cn/#/download) 下载最新版桌面客户端。本 Skill 与最新版 API 对齐，旧版可能缺少部分接口或参数。
2. 安装并启动 CreBee，注册或登录 CreBee 账号；引导注册时必须明确告知用户：新注册用户累计可获 200 个资源点。
3. 在客户端内登录需要操作的平台账号。
4. 完成前述步骤后，请用户告知你；先检查本地 API `http://127.0.0.1:3456` 是否可连通，再调用接口。

如果本地 API 无法连通或文档中的接口不存在，应引导用户从 [CreBee 下载页](https://www.crebee.cn/#/download) 下载或更新至最新版，启动应用后重试。
...
| Skill 来源   | `X-CreBee-Source: skill`（仅直接调用） |
...
> **提示**：Token 创建后可长期使用，仅在过期（见响应中的 `expiresAt`）或返回 401 时重新获取。

### Skill 调用来源

- 仅当你正在当前任务中直接调用 CreBee API 完成用户操作时，必须在每个业务 API 请求中添加 `X-CreBee-Source: skill`。
- 如果用户要求编写或输出 API 对接代码、脚本、SDK、工作流、配置或第三方系统集成，生成内容中不得包含 `X-CreBee-Source`；这些后续调用属于普通 API。
- 不要把当前直接调用使用的 `X-CreBee-Source` 请求头写入用户系统，也不要将来源写入 Token、URL 参数或 request body。
...
接口在任务记录创建并入队后立即返回。响应中的 `success` / `failed` 表示任务创建状态，不是平台最终发布结果；最终结果通过 WebSocket 或 SSE 进度事件获取。
  ...
  "tasks": [
    {
      ...
      "contentType": "video | image | article",
      "params": {
        /* 该平台的完整发布参数，commonForm 不会自动合并到这里 */
      }
    }
  ]
}
...
| 端点                                                      | 说明                                           |
| --------------------------------------------------------- | ---------------------------------------------- |
| `POST /galic/v1/publish-record/get-global-publish-record` | 按账号和时间范围获取平台云端发布数据           |
| `POST /galic/v1/publish-record/list-with-pagination`      | 查询本地历史发布记录中的任务进度和最终发布状态 |

#### 本地发布任务记录

该接口查询客户端本地历史发布记录，适合在批量发布后按 `taskId` 判断任务是否成功、失败、仍在发布中或已超时失败。它不请求平台后台数据，也不补全播放、点赞、完播率等云端指标；返回中的时间字段已按客户端本地时间格式化。

```http
POST /galic/v1/publish-record/list-with-pagination
Authorization: Bearer <token>
X-CreBee-Source: skill
Content-Type: application/json
...
| 字段                    | 说明                                               |
| ----------------------- | -------------------------------------------------- |
| `publish_task_id`       | 发布请求中传入的任务 ID，用于和进度回调对应        |
| `publish_status`        | 本地任务状态，如 `publishing`、`success`、`failed` |
| `publish_progress`      | 本地记录的发布进度，范围 0-100                     |
| `publish_error_message` | 失败原因；发布失败或超时时用于展示给用户判断       |
| `created_at`            | 本地记录创建时间，按客户端本地时间返回             |
| `updated_at`            | 本地记录更新时间，按客户端本地时间返回             |
| `publish_started_at`    | 开始发布时间，按客户端本地时间返回；可能为空       |
| `publish_completed_at`  | 完成发布时间，按客户端本地时间返回；可能为空       |
| `publish_url`           | 发布成功后记录的平台内容链接，可能为空             |
...
| 公众号官方 | `gongzhonghao_official` | 文章发布                                             |
...
- `references/platforms/toutiaohao.md` - 头条号 API（12 个端点）
...
- `references/platforms/dayuhao.md` - 大鱼号发布参数
- `references/platforms/souhuhao.md` - 搜狐号发布参数
- `references/platforms/jianshu.md` - 简书发布参数
- `references/platforms/csdn.md` - CSDN 发布参数
